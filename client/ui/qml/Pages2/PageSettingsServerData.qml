...
PageType {
    ...
    QtObject {
        ...
        readonly property string title: qsTr("Проверить сервер на ранее установленные службы Скрытой сети")
    ...
    QtObject {
        ...
        readonly property var clickedHandler: function() {
            ...
            var descriptionText = qsTr("Все установленные службы Скрытой сети останутся на сервере.")
            var yesButtonText = qsTr("Continue")
            var noButtonText = qsTr("Cancel")
    ...
    QtObject {
        ...
        property bool isVisible: root.isServerWithWriteAccess
        readonly property string title: qsTr("Удалить программное обеспечение Скрытой сети с сервера")
        readonly property string description: ""
        readonly property var tColor: AmneziaStyle.color.vibrantRed
        readonly property var clickedHandler: function() {
            var headerText = qsTr("Удалить программное обеспечение Скрытой сети с сервера?")
            ...
            var yesButtonFunction = function() {
                if (ServersUiController.isDefaultServerCurrentlyProcessed() && ConnectionController.isConnected) {
                    PageController.showNotificationMessage(qsTr("Нельзя удалить программное обеспечение Скрытой сети при активном соединении"))
                } else {
